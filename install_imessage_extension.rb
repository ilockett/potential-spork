...
target_product_name = 'ILiMessage'
targets = []
project.targets.each do |target|
  targets.push(target.product_name)
end
if targets.include? target_product_name
...
target = project.new_target(:messages_extension, target_product_name, :ios, '11.0')
...
target.build_configurations.each do |configuration|
  ...
  configuration.build_settings['PRODUCT_NAME'] = target_product_name
  configuration.build_settings['INFOPLIST_FILE'] = __dir__ + '/Info.plist'
  ...
  configuration.build_settings['FRAMEWORK_SEARCH_PATHS'] = __dir__
...
extension_group = project.new_group(target_product_name)
...
extension_dir = Dir[__dir__ + '/Sources/*']
